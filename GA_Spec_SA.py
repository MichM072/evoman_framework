...
class GASpecialistSA:
    ...
    def log_initial_population_fitness(self) -> None:
        ...
        if self.run == 0:  # Only init the header if it is the first run
            with open(
                self.experiment_name + f"/{self.mode.lower()}_results.txt", "a"
            ) as file_aux:
                file_aux.write(
    ...
    def log_generation_statistics(
        ...
        with open(
            ...
            file_aux.write(
                f'\n{self.run:<10} {generation:<10} {best_fitness:<10.6f} {record["avg"]:<10.6f} {record["std"]:<10.6f} {self.enemy:<10} {self.mutation_prob:<10.6f} {self.t:<10.6f}'
            )
    ...
    def save_results(self, best) -> None:
        ...
        np.savetxt(
            ...
            + f"/enemy_{self.enemy}/{self.mode.lower()}_best_{self.run}.txt",
            best[0],
        )
        with open(
            self.experiment_name
            + f"/enemy_{self.enemy}/{self.mode.lower()}_best_fitness.txt",
            "a",
        ) as f:
    ...
    def run_experiment(self, enemy: int, mode: str, run: int, best_ind_idx: int):
        ...
        if self.mode == "Train" or self.mode == "Tune":
            ...
            best_ind = np.loadtxt(
                self.experiment_name
                + f"/enemy_{self.enemy}/train_best_{best_ind_idx}.txt"
            )
            if self.verbose:
